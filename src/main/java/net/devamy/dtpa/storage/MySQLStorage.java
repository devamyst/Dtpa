...
public class MySQLStorage implements Storage {
    ...
    private final Map<UUID, Set<String>> requestsBySender = new ConcurrentHashMap<>();
    private final Map<UUID, Set<String>> requestsByReceiver = new ConcurrentHashMap<>();

    public MySQLStorage(DTPA plugin, String host, int port, String database,
    ...
    private void indexRequest(TpaRequest req) {
        String key = req.getSenderUuid() + ":" + req.getReceiverUuid();
        requestsCache.put(key, req);
        requestsBySender.computeIfAbsent(req.getSenderUuid(), k -> ConcurrentHashMap.newKeySet()).add(key);
        requestsByReceiver.computeIfAbsent(req.getReceiverUuid(), k -> ConcurrentHashMap.newKeySet()).add(key);
    }

    private void deindexRequest(UUID senderUuid, UUID receiverUuid) {
        String key = senderUuid + ":" + receiverUuid;
        requestsCache.remove(key);
        Set<String> sSet = requestsBySender.get(senderUuid);
        if (sSet != null) { sSet.remove(key); if (sSet.isEmpty()) requestsBySender.remove(senderUuid); }
        Set<String> rSet = requestsByReceiver.get(receiverUuid);
        if (rSet != null) { rSet.remove(key); if (rSet.isEmpty()) requestsByReceiver.remove(receiverUuid); }
    }
    ...
    public void removeRequest(UUID senderUuid, UUID receiverUuid) {
        deindexRequest(senderUuid, receiverUuid);
    ...
    @Override
    public TpaRequest removeAndGetRequest(UUID senderUuid, UUID receiverUuid) {
        String key = senderUuid + ":" + receiverUuid;
    ...
    public TpaRequest getRequest(UUID senderUuid, UUID receiverUuid) {
        ...
        TpaRequest cached = requestsCache.get(key);
        if (cached != null) {
            if (cached.getExpiresAt() == 0 || cached.getExpiresAt() > System.currentTimeMillis()) {
                return cached;
            }
            deindexRequest(senderUuid, receiverUuid);
        }
    ...
    public List<TpaRequest> getRequestsBySender(UUID sender) {
        Set<String> keys = requestsBySender.get(sender);
        if (keys == null || keys.isEmpty()) return List.of();
        List<TpaRequest> list = new ArrayList<>(keys.size());
        for (String k : keys) {
            TpaRequest req = requestsCache.get(k);
    ...
    public List<TpaRequest> getRequestsByReceiver(UUID receiver) {
        Set<String> keys = requestsByReceiver.get(receiver);
        if (keys == null || keys.isEmpty()) return List.of();
        List<TpaRequest> list = new ArrayList<>(keys.size());
        for (String k : keys) {
            TpaRequest req = requestsCache.get(k);
    ...
    public void cleanupExpired(long cutoffEpochMs) {
        ...
        requestsCache.entrySet().removeIf(e -> {
            ...
            if (exp > 0 && exp <= cutoffEpochMs) {
                TpaRequest req = e.getValue();
                deindexRequest(req.getSenderUuid(), req.getReceiverUuid());
                return true;
            }
            return false;
        });
