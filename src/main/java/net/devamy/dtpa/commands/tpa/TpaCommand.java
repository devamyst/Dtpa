...
@CommandAlias("tpa")
@CommandPermission("dtpa.command.tpa")
public class TpaCommand extends BaseCommand {
    ...
    public void command(Player player, @Optional String targetName) {
        ...
        if (plugin.getTpaManager().isTpaDisabled(target.getUniqueId())) {
            CC.send(player, "&c" + target.getName() + " has DTPA disabled.");
            return;
        }
        if (plugin.getTpaManager().hasPendingRequest(player, target)) {
            CC.send(player, "&cYou already have a pending DTPA request to &e" + target.getName() + "&c.");
            return;
        }
    ...
    static void handleDirectSend(DTPA plugin, Player sender, Player receiver, TpaType type) {
        ...
        if (plugin.getTpaManager().isAutoAccept(receiver.getUniqueId())) {
            ...
            if (!plugin.getTpaManager().isConfirmDisabled(receiver.getUniqueId())) {
                new net.devamy.dtpa.gui.AcceptGui(sender, receiver).open();
            }
        }
    }
}
