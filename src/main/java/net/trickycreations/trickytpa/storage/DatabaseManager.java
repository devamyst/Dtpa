...
public final class DatabaseManager {
    ...
    public DatabaseManager(TrickyTPA plugin) {
        this.plugin = plugin;
    }
        ...
        try {
            switch (type) {
                case "mysql" -> {
                    ...
                    String db = config.getString("mysql.database", "trickytpa");
                    ...
                    String prefix = config.getString("mysql.table_prefix", "trickytpa_");
                    storage = new MySQLStorage(plugin, host, port, db, user, pass, ssl, pool, timeout, prefix);
                }
                case "h2" -> {
                    String filename = config.getString("h2.filename", "trickytpa");
                ...
                default -> { // sqlite
                    String filename = config.getString("sqlite.filename", "trickytpa.db");
            ...
            // Fallback to SQLite
            try {
                String jdbcUrl = "jdbc:sqlite:" + plugin.getDataFolder() + "/trickytpa.db";
                storage = new EmbeddedStorage(plugin, jdbcUrl);
                storage.init();
